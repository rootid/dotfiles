...
# Custom Claude Code statusline - Created: 2026-09-22T03:11:11.512Z
# Theme: detailed | Colors: false | Features: directory, git, model, context, usage, tokens, burnrate, session
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  cc_version=$(echo "$input" | grep -o '"version"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed 's/.*:[[:space:]]*"\([^"]*\)".*/\1/')
  # Output style is nested
  output_style=$(echo "$input" | grep -o '"output_style"[[:space:]]*:[[:space:]]*{[^}]*"name"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/.*"name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')
fi

# Patched: upstream cc-statusline v1.4.0 calls these but never defines them
# when colors are disabled, which prints "command not found" on every render.
dir_color() { :; }
model_color() { :; }
version_color() { :; }
cc_version_color() { :; }
