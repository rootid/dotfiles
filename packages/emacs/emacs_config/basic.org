* Errors
⛔ Error (use-package): org-roam/:config: Opening directory: No such file or directory, /Users/vmat/org-roam-notes
⛔ Warning (emacs): The package embark-consult should be installed if you use both Embark and Consult
⛔ Warning (emacs): The package embark-consult should be installed if you use both Embark and Consul
...
* Shortcuts to Open Browser - TODO - Create different types of workflows with one click and Hydra
...
#+BEGIN_SRC emacs-lisp
          ...
          '(("c" "Capture Task, Book, Media")
            ...
            ("ni" "Add Idea to Marketplace" entry
             ...
             (file "~/templates/tpl_idea.txt")
               :empty-lines-before 1)
            ("t" "Self Instrumentation Entry") 
            ("tc" "Craving Tracker" table-line (file+headline "~/tracker/tracker.org" "Craving Tracker") 
             (file "~/templates/tpl_craving.txt") :kill-buffer t)
            ("tp" "Pain,Pleasure Tracker" table-line (file+headline "~/tracker/tracker.org" "Pain,Pleasure Tracker") 
             (file "~/templates/tpl_pain_pleasure.txt") :kill-buffer t)
            ("tb" "Self Binding Tracker" table-line (file+headline "~/tracker/tracker.org" "Self Binding Tracker") 
             (file "~/templates/tpl_dopamine.txt") :kill-buffer t)
            ("te" "Energy Tracker" table-line (file+headline "~/tracker/tracker.org" "Energy Tracker") 
             (file "~/templates/tpl_energy_tracker.txt") :kill-buffer t)
            ("tw" "Water Tracker" table-line (file+headline "~/tracker/tracker.org" "Water Tracker") 
             (file "~/templates/tpl_water_tracker.txt") :kill-buffer t)
            ("ts" "Typing Speed" table-line (file+headline "~/tracker/tracker.org" "Typing Speed Tracker") 
             (file "~/templates/tpl_typing_speed_tracker.txt") :kill-buffer t)
            ("j" "Create Journal Entry") 
            ("jd" "Daily Journal Entry" entry (file+olp+datetree "~/tracker/journal.org") "* %?\nEntered on %U\n  %i\n  %a")
            ("jq" "Daily Journal Entry" table-line (file+headline "~/tracker/tracker.org" "Craving Tracker") 
             (file "~/templates/tpl_craving.txt") :kill-buffer t)
            ("p" "Create a daily plan") 
            ("pr" "Daily plan" DailyPlan 
             (file+function "~/review/daily_review.org" org-reverse-datetree-goto-date-in-file) 
             (file "~/templates/tpl_dailyplan.txt") 
             :clock-in t :clock-resume t :prepend t :empty-lines-after 2)))

    ;; Refile using - C-c C-w
    ;;(setq org-refile-targets
    ;;      '((("~/gtd/projects.org") :regexp . "\\(?:\\(?:Note\\|Task\\)s\\)") ;; Deleted archieved
    ;;        (("~/done/done_projects.org") :maxlevel . 3)))
  ...
  ;;      "/Users/vmat/Dropbox/plain_docs/area/v1/denote/books" "\\.org$"))
  ;;     (org-overriding-columns-format
  ;;     "%40item %10todo %10author %20Links")
...
** Emacs Theme
- Why?
Limited number of themes are avaiable
- How?
Go to page to view the avaiable theme and try one at a time
#+BEGIN_SRC emacs-lisp
...
(use-package org-roam
  ...
  :custom
  (org-roam-directory (file-truename "~/Dropbox/plain_docs/area/v1/roam/projects")
  (org-roam-completion-everywhere t))
  :bind (("C-h l" . org-roam-buffer-toggle)
         ("C-h d" . org-roam-find-directory)
         ("C-h /" . org-roam-node-find)
         ("C-h 0" . org-roam-capture)
         ("C-h i" . org-roam-node-insert)
         ("C-h g" . org-roam-graph)
         ;; Dailies
  ...
  (org-roam-db-autosync-mode) ;; Create/build org roam cache 
  (org-roam-setup)
  ;; If using org-roam-protocol
  (require 'org-roam-protocol))

;; Resolve symlink, impacts performance
(setq find-file-visit-truename t)
#+END_SRC 

** Helper method to switch the directories

#+begin_src emacs-lisp
(defun vs_org_roam/switch_project ()
    "Switch org roam to project"
    (interactive)
    (setq org-roam-directory "~/Dropbox/plain_docs/area/v1/roam/projects")  
    (setq org-roam-db-location "~/Dropbox/plain_docs/area/v1/roam/projects/org-roam.db"))

(defun vs_org_roam/switch_notes ()
    "Switch org roam to project"
    (interactive)
    (setq org-roam-directory "~/Dropbox/plain_docs/area/v1/roam/draft")  
    (setq org-roam-db-location "~/Dropbox/plain_docs/area/v1/roam/draft/org-roam.db"))
#+end_src
